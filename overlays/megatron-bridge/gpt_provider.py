...
class GPTModelProvider(TransformerConfig, ModelProviderMixin[MCoreGPTModel]):
    ...
    def provide(self, pre_process=None, post_process=None, vp_stage=None) -> MCoreGPTModel:
        ...
        """
        # zhe-li fp8-base port (GLM_FP8_BASE=1): build the frozen base as blockwise-fp8
        # so the full 744B fits at 128k on 4 nodes (bf16 base ~99 GB/rank OOMs; fp8 ~49 GB).
        # fp8_param requires config.fp8 (fp8 compute mode) -> set e4m3 + blockwise recipe
        # HERE, before the transformer_layer_spec is built (it reads fp8 to pick TE fp8 layers).
        import os as _os
        if _os.environ.get("GLM_FP8_BASE") == "1":
            from megatron.core.enums import Fp8Recipe as _Fp8Recipe
            if self.fp8 is None:
                self.fp8 = "e4m3"
            self.fp8_recipe = _Fp8Recipe.blockwise
            self.fp8_param = True
            print(f"[GLM_FP8_BASE] fp8-base ON: fp8={self.fp8} recipe={self.fp8_recipe} "
                  f"fp8_param={self.fp8_param} -> building blockwise-fp8 base", flush=True)
        ...
        # zhe-li fp8-base: wrap construction in fp8_model_init (is_init=True) so TE linears
        # allocate Float8BlockwiseQTensor weight params (nullcontext when fp8_param is off).
        if getattr(self, "fp8_param", False):
            from megatron.core.fp8_utils import get_fp8_context as _get_fp8_context
            _fp8_init_ctx = _get_fp8_context(self, is_init=True)
        else:
            _fp8_init_ctx = contextlib.nullcontext()
        with model_init_device_context(), _fp8_init_ctx:
            model = MCoreGPTModel(
                self,
