...
class GLM5Bridge(MegatronModelBridge):
    ...
    def maybe_modify_loaded_hf_weight(self, hf_param, hf_state_dict: "_Mapping[str, torch.Tensor]"):
        """GLM-5.2 stores linear weights as blockwise fp8 (float8_e4m3fn) + a
        separate ``<key>_scale_inv`` per 128x128 block. The base bridge cast fp8
        straight to bf16 WITHOUT the scale (loading the base ~6000x too large ->
        NaN). Apply the blockwise dequant here (mirrors minimax_m2)."""
        if isinstance(hf_param, dict):
            return {k: self._glm5_load_and_dequant(v, hf_state_dict) for k, v in hf_param.items()}
        return self._glm5_load_and_dequant(hf_param, hf_state_dict)

    @staticmethod
    def _glm5_load_and_dequant(key: str, hf_state_dict) -> torch.Tensor:
        w = hf_state_dict[key]
