...
- **Grid:** 1 node = TP8/PP1/EP8/ETP1; 2 nodes = TP8/PP1/EP16/ETP1. CP=1 (DSA does not support context parallelism on the public stack); PP=1 (miles LoRA weight-sync requires it). DSA via `tilelang` backend + `--qkv-format thd` (recompute-safe).
...
3. **Offload is a genuine, powerful lever** — it took 30L/128k from ~11.5 GB over to ~0.2 GB over, entirely on public code.
4. **Full 744B/128k on the public bf16 stack is not reachable by adding nodes.** It needs the memory tricks the private fork provides: NVFP4/FP8 base (shrinks the un-shardable base further), context parallelism (shards the sequence dimension — blocked here by DSA), and/or chunked loss.

## 7. Paths Forward (not executed)

- **Chunked cross-entropy in `miles/backends/training_utils/loss.py`** — compute the SFT loss over vocab tiles so the full logits are never materialized. Frees well over the 0.23 GB needed; unlocks ~30 layers on 2 nodes. Correctness-sensitive; needs one verification run.
...
*Raw data and per-run memory numbers: `MEMSCALE_RESULTS.md`. Reproduction: `HANDOFF.md`. Scripts: `scripts/memscale-1node.sbatch`, `scripts/sft-128k-glm744b.sbatch` (`OFFLOAD_LAYERS`/`CE_FP16` env toggles), `scripts/memscale-30L-2node-offload.sbatch`.*
